...
### 用户类型 C：偏轻社交与灵感需求的社区用户

- 典型场景：情绪疲惫、思路卡壳、想获得一句外部刺激。
- 核心诉求：不想深度社交，只想要一次轻盈、有趣、低负担的碰撞。
...
5. **轻重分层**：既有严肃匹配主线，也有轻量玩法承担传播与日活。

## 5. 核心用户流程

1. 用户完成 `W-MBTI 40 题标准量表`。
2. 系统生成 `双核名片`，展示用户在生活态与职场态中的差异化协作画像。
3. 用户发起组队/合伙匹配，或在广场中表达连接意愿。
4. 双方互相看中后，不直接交换联系方式，而是进入 `A2A 赛博沙盘`。
5. 双方 Agent 围绕真实职场议题进行多轮推演、立场表达与策略博弈。
6. 系统输出 `兼容性结论`：
   - 可继续深入
   - 可合作但需明确边界
   - 建议终止连接
7. 若通过，则生成 `专属协作说明书` 与 `知乎护身符建议`。
8. 双方确认后，执行 `一键交换现实名片`。
9. 未进入重度匹配链路的用户，也可以使用 `赛博灵感抽卡机` 获得轻量互动。
...
| 双核人格生成器 | 双核名片展示 | 生成兼具极客感与文学感的双态角色小传 | 卡片页展示：类型、双态标题、优势、雷区、沟通建议 | 可分享海报、动态角色皮肤、时间线案例 | 文案质量、视觉表达风格统一 |
| A2A 赛博沙盘 | 双盲谈判引擎 | 双方 Agent 在后台进行多轮问答与协商 | 基于双方人格画像和意图的多轮 Prompt 编排，输出回合记录与总结 | 多 Agent 厅、可视化对战、历史回放 | Agent 质量稳定性、成本控制 |
...
| 赛博灵感抽卡机 | 借个“破局” Buff | 与互补路人 Agent 短暂匹配，获得一句提振 | 轻量匹配 + 单轮生成 + 随机数字 / 短句输出 | 广场模式、连击彩蛋、组队导流 | 内容重复度、轻互动质量 |
| 赛博灵感抽卡机 | 一句话灵感交汇 | 不做沉重社交，只做瞬时启发 | 单回合匿名交汇卡片 | 可收藏、可回访、可转为深度匹配 | 轻玩法和主链路之间的转化设计 |
...
3. 知乎实时数据接入方式是否会提供官方可用源，否则继续长期采用内容池模式。
4. 页面层是否要在下一轮补齐真正的表单/按钮交互，而不是以 API 驱动为主。

## 15. 一句话总结

**双核职场** 的核心不是“测出你是什么人”，而是“提前验证你和谁能一起把事做成”。它用双核人格理解个体，用 A2A 沙盘验证关系，用协作契约交付结果，用轻量抽卡扩散活性。
